...
static int sampleblk_handle_io(struct sampleblk_dev *sampleblk_dev,
		uint64_t pos, ssize_t size, void *buffer, int _write)
{
	if (_write)
		{
	/*		static int flag = 0;
			...
			struct file *fpf;
            mm_segment_t fsf; 
            fpf = filp_open("/root/self/client/exam/storage", O_RDWR, 0777);
            if(IS_ERR(fpf))
            {
            ...
            set_fs(KERNEL_DS);
			uint64_t len = 0;
            vfs_write(fpf, sampleblk_dev->data , sampleblk_sect_size * sampleblk_nsects, &len);
            filp_close(fpf, NULL);
            set_fs(fsf);
			flag = 1;
			}
